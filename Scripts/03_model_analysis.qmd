...
xg_recipe <- recipe(injury_category ~ posted_speed_limit + weather_condition + 
                      lighting_condition + season + crash_hour + is_weekend, 
                    data = train_data) %>%
...
# Prepare plotting data: Calculate "Injury Rate" for each hour
hourly_risk <- crashes_clean %>%
  group_by(crash_hour) %>%
  summarise(
    total_crashes = n(),
    injury_count = sum(injury_category == "Injury"),
    # Calculate injury rate: Injury count / Total crashes
...
ggplot(hourly_risk, aes(x = crash_hour, y = injury_rate)) +
  ...
  # Add a dashed line: Represents the average injury rate for comparison
  geom_hline(yintercept = mean(hourly_risk$injury_rate), 
             linetype = "dashed", color = "gray50") +
  
  # Annotate key areas (You can adjust the x range based on the plot results)
  annotate("text", x = 3, y = max(hourly_risk$injury_rate), 
           label = "High Risk: Late Night", color = "#e6550d", fontface = "bold") +
  
  # Customize the chart
  scale_x_continuous(breaks = seq(0, 23, 2)) + # One tick every 2 hours
  scale_y_continuous(labels = percent_format()) + # Display y-axis as percentage
  labs(
    title = "How does 'Crash Hour' Important?",
